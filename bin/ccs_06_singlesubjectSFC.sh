...
do
	...
	3dROIstats -quiet -mask_f2short -mask ${seed} ${res_dir}/${rest}.sm${FWHM}.mni152.nii.gz > ${seed_ts_dir}/${seed_name}.1D 2>&1
	## 2. Compute voxel-wise correlation with Seed Timeseries      
        ...
        3dfim+ -input ${res_dir}/${rest}_pp_sm${FWHM}.nii.gz -ideal_file ${seed_ts_dir}/${seed_name}.1D -out Correlation -bucket ${seed_name}_Rmap.nii.gz 2>&1
	## 3. Z-transform correlations		
	echo Z-transforming correlations for seed ${seed_name}
	rm -f ${seed_name}_Zmap.nii.gz
	3dcalc -a ${seed_name}_Rmap.nii.gz -expr 'log((a+1)/(1-a))/2' -prefix ${seed_name}_Zmap.nii.gz 2>&1
	## 4. Computing Z-statistics
	echo Computing Z-statistical map for seed ${seed_name}
	rm -f ${seed_name}_Zstat.nii.gz
	3dcalc -a ${seed_name}_Zmap.nii.gz -expr 'a*sqrt('${nvols}'-3)' -prefix ${seed_name}_Zstat.nii.gz 2>&1
