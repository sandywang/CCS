...
if exist('options.mat'),
 ...
 threshold=0.0;  % visualization threshold
 linlogtfr=0;    % tfr visualization scale : 0 for linear 1 for logarithmic
 linlogspec=0;   % spectrum visualization scale
 sigenveloppe=0; % signal enveloppe visualization

 levelnumb=256;   % number of levels in the contour plot
 colmap=1;       % colormap index
 
 display=2;      % display index
 
 isgridsig=1;    % grid on signal
 isgridspec=1;   % grid on spectrum
 isgridtfr=0;    % grid on tfr
 
 issig=1;        % display signal
 isspec=1;       % display spectrum
 iscolorbar=1;   % display colorbar
