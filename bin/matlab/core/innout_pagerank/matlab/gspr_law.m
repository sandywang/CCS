function [x flag reshist] = gspr_law(P,a,v,tol,maxit,resid,verbose,x0,xstar)
...
%
% x=gspr(P,a,v,tol,maxit,resid,verbose,x0,xstar) specifies all options
...
%
% This version of gauss-seidel corresponds to the version implemented by
% the LAW package.
%
...
function delta=prresid(x,P,a,v,dsum)
    %% Compute the residual of the eigensystem
    %  r = a*P'*x + a*(d'*x)*v + (1-a)*(e'*x)*v - x
    % alterates are
    %  linear system: r = a*P'*x + a*(d'*x)*v + (1-a)*v - x
    %  exact 1-norm : y = x/norm(x,1); r = a*P'*y + a*(d'*y)*v + (1-a)*v - x
    %  
    h = a*(P'*x);
    w=a*dsum + (1-a)*csum(x);
    h=h+w*v; 
