...
P = normout(A);
n = size(P,1);
[x ignore ignore Ps] = gssweeppr(ones(n,1),P);
Ps.P = P;
...
for bi=1:length(betas)
    ...
    for ei=1:length(etas)
        ...
        [x flag hist] = inoutgspr(Ps,alpha,[],tol,maxit,[],beta,eta,0);
        ...
        save([graph '-' astr '-inoutgs.mat'],'results','mresults','rnum');
    end
end
...
xlim(xl); axis square; box on; etasstr=cellstr(num2str(etas(2:end)','%4.0e'));
etasstr = cellfun(@(x) ['\eta = ' x], etasstr,'UniformOutput',false);
legend('power',etasstr{:},'Location','NorthEast'); 
xlabel('\beta'); ylabel('Sweeps');
title(sprintf('%s, \\alpha=%4.2f', graph, alpha));
saveas(gcf,[graph '-' astr '-inoutgs-betas.fig'],'fig');
print(gcf,[graph '-' astr '-inoutgs-betas.eps'],'-depsc2','-cmyk');

%%
bsubset=[3 7 11 15];
figure(2); clf; hold on;
e2=[1 1]; e3=[e2 1]; xl=[min(etas(2:end)),max(etas(2:end))];
line(xl, e2*mresults(1,1), 'Color', 0.7*e3, 'LineWidth', 0.75);
stys={'kd:','ks-','ko--','kv-'};
for i=1:length(bsubset)
    plot(etas(2:end),mresults(bsubset(i),2:end),stys{i},'MarkerSize',4);
end
...
xlabel('\eta'); ylabel('Sweeps'); 
title(sprintf('%s, \\alpha=%4.2f', graph, alpha));
saveas(gcf,[graph '-' astr '-inoutgs-etas.fig'],'fig');
print(gcf,[graph '-' astr '-inoutgs-etas.eps'],'-depsc2','-cmyk');
