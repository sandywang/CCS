function [x flag reshist phist pstart D emag]=inoutpr(P,a,v,tol,maxit,b,itol,verbose)
...
% 2008-05-30: fixed eigenvalue sorting
...
<<<<<<< .mine
[V D] = eig(P'); [ignore pd]=sort(-abs(diag(D))-eps(1)*sign(diag(D))); 
V=V(:,pd); D=D(pd,pd); emag = zeros(n,maxit);
=======
[V D] = eig(P'); evals = diag(D); %evals = round(10000*evals)/10000;
[ignore pd]=sort(-abs(evals+100*eps)); 
V=V(:,pd); D=D(pd,pd); emag = zeros(n,maxit);
>>>>>>> .r776
phist = zeros(n,maxit); xstar = (speye(n)-a*P')\(v.*ones(n,1)); 
xstar=xstar./norm(xstar,1); ustar = V\xstar;

x=zeros(n,1)+v; y=P'*x;y=y+(sum(x)-sum(y))*v; nm=1; f=a*y;f=f+(1-a)*v;f=f-x; 
u = V\x; phist(:,nm) = u-ustar; emag(:,nm)=u;
...
while nm<maxit && dlta>tol
    ...
    while nm+ii<maxit && d>itol,
        ...
        u = V\(a*y + (1-a)*v); phist(:,nm+ii) = u-ustar; emag(:,nm+ii)=u;
    end
...
while nm<maxit && dlta>tol
    ...
    u = V\(x./norm(x,1)); phist(:,nm)=u-ustar; emag(:,nm)=u;
    if verbose, fprintf('inout (pow) : m=%7i d=%8e r=%8e\n',nm,dlta,dlta/dp); 
        dp=dlta; end
end
x=x./norm(x,1); flag=dlta>tol; reshist=reshist(1:nm); phist=phist(:,1:nm); emag=emag(:,1:nm);
