...
[V D] = eig(P'); [ignore pd]=sort(-abs(diag(D))); V=V(:,pd); D=D(pd,pd);
emag = zeros(n,maxit);
...
dlta=norm(f,1);x=x-b*y;reshist=[dlta;zeros(maxit-1,1)]; ihist=zeros(maxit,1);
while nm<maxit && dlta>tol
    ...
    figure(3); semilogy(ihist(1:ii)); axis square; pause(5);
