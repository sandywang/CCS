...
%
% [weight overlap]=mwmround(x,S,w,li,lj,n,m) returns the weight and overlap
...
weight = sum(w(mi)); overlap = (mi'*(S*mi))/2; 

if nargout<3, varargout{1} = weight; varargout{2} = overlap; 
else 
    varagout{1} = ma; varargout{2} = mb; varargout{3} = mi; 
