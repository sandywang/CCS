...
% 2008-05-14: Variant 1... test a possibly equivalent form of the iteration
...
while nm<maxit && dlta>tol
    ...
    while nm+ii<maxit && d>itol,
        ...
        y = (P'*yk); y=y+(sum(x)-sum(y))*v; ii=ii+1;
        ykn = b*y + (1-b)*v;
        d = norm(ykn-yk,1);
        yk = ykn;
    end
    x=xn-y0+yk;
    y = (P'*x); y=y+(sum(x)-sum(y))*v; ii=ii+1;
    if ii<2, x=a*y; x=x+(1-a)*v; y=y-x; break; end % no mult => no hist updte
