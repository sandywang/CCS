...
x=zeros(n,1)+v; reshist=zeros(maxit,1); dlta=2; ii=0; nm=0;
y=P'*x;y=y+(sum(x)-sum(y))*v; f=a*y;f=f+(1-a)*v;f=f-x; 
dlta=norm(f,1); reshist(nm+1:nm+ii)=dlta; nm=nm+ii; 
f=(a-b)*y; f=f+(1-a)*v; 
while nm<maxit && dlta>tol
    y=P'*x;y=y+(sum(x)-sum(y))*v; f=a*y;f=f+(1-a)*v;f=f-x; 
    dlta=norm(f,1); reshist(nm+1:nm+ii)=dlta; nm=nm+ii; 
    f=(a-b)*y; f=f+(1-a)*v; % f=(a-b)*y+vt;
%     while nm+ii<maxit && d>itol,
%         x=b*y; x=x+f; y=P'*x; y=y+(sum(x)-sum(y))*v;ii=ii+1; % x=b*y+f;y=Pd'*x
%         x=x-b*y; x=x-f; d=norm(x,1);   % x=x-b*y-f;
%     end
    [x flag histgs]=gspr(P,b,f./(1-b),itol,maxit-nm,true,verbose,[],x./csum(x)); % f./(1-b)
    ii = length(histgs); 
    y=P'*x;y=y+(sum(x)-sum(y))*v; f=a*y;f=f+(1-a)*v;f=f-x; 
    dlta=norm(f,1); reshist(nm+1:nm+ii)=dlta; nm=nm+ii; 
    f=(a-b)*y; f=f+(1-a)*v;
    if verbose, fprintf('inout (out) : m=%7i d=%8e\n',nm,dlta); end    
    if ii<2, break; end % no mult => no hist updte
