...
if [ $# -lt 4 ]; 
then
    	echo -e "\033[47;35m Usage: $0 analysis_dir subjects_list dti_dir_name clean_bool subjects_dir \033[0m"
	exit
fi
...
do
	...
	if [ ! -e $output_dir/coronal.png ]; then
	...
	# Draw with freeview and save axial slices
        if [ ! -e $output_dir/axial.png ]; then
	...
	# Draw with freeview and save sagittal slices
        if [ ! -e $output_dir/sagittal.png ]; then
	...
	title=${subject}.ccs.dti.anatomy
	convert -font helvetica -fill white -pointsize 36 -draw "text 570,200 '$title'" ${output_dir}/summary.png ${output_dir}/summary.png
done
