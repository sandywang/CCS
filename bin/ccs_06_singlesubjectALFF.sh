...
## Updated by Ting Xu, Mar 23, 2014
##########################################################################################################################
...
if [ $# -lt 9 ];
then
        echo -e "\033[47;35m Usage: $0 analysis_dir subject_list rest_name TR anat_dir_name func_dir_name do_refine_reg standard_template gs_removal\033[0m"
        exit
fi
...
func_reg_dir=${dir}/${subject}/${func_dir_name}/reg
if [ "${gs_removal}" = "true" ]
then
	func_dir=${dir}/${subject}/${func_dir_name}/gs-removal
else
	func_dir=${dir}/${subject}/${func_dir_name}
fi
func_mask_dir=${dir}/${subject}/${func_dir_name}
...
then
	...
	n_vols=`fslnvols ${func_dir}/${rest}_pp_nofilt_sm${FWHM}.nii.gz` ; echo "there are ${n_vols} vols"
	...
	if [ ${MOD} -eq 1 ]
	then
    		fslroi ${func_dir}/${rest}_pp_nofilt_sm${FWHM}.nii.gz prealff_func_data.nii.gz 1 ${N}
	fi
	if [ ${MOD} -eq 0 ]
	then
    		cp ${func_dir}/${rest}_pp_nofilt_sm${FWHM}.nii.gz prealff_func_data.nii.gz
	fi
	...
	do
		echo ${metric}
		fslstats ${metric}.nii.gz -k ${func_mask_dir}/${rest}_pp_mask.nii.gz -m > mean_${metric}.txt ; 
		mean=$( cat mean_${metric}.txt )
		fslstats ${metric}.nii.gz -k ${func_mask_dir}/${rest}_pp_mask.nii.gz -s > std_${metric}.txt ; 
		std=$( cat std_${metric}.txt )
		echo $mean $std 
		fslmaths ${metric}.nii.gz -sub ${mean} -div ${std} -mas ${func_mask_dir}/${rest}_pp_mask.nii.gz ${metric}_Z.nii.gz
		...
		then
			...
			applywarp --ref=${standard_template} --in=${ALFF_dir}/${metric}.nii.gz --out=${ALFF_dir}/fnirt_${metric}.nii.gz --warp=${anat_reg_dir}/highres2standard_ref_warp --premat=${func_reg_dir}/example_func2highres.mat
		else
			...
			applywarp --ref=${standard_template} --in=${ALFF_dir}/${metric}.nii.gz --out=${ALFF_dir}/fnirt_${metric}.nii.gz --warp=${anat_reg_dir}/highres2standard_warp --premat=${func_reg_dir}/example_func2highres.mat
		fi
	done
	## 7. Clean up
