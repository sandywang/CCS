...
then
	## 1. Temporal filtering
        ...
        then
                for hemi in lh rh
                do
                        mri_vol2surf --mov ${gsFC_dir}/${rest}_pp_sm0.nii.gz --reg ${func_reg_dir}/bbregister.dof6.dat --trgsubject ${fsaverage} --interp trilin --projfrac 0.5 --hemi ${hemi} --o ${gsFC_dir}/${rest}.pp.sm0.${fsaverage}.${hemi}.nii.gz --noreshape --cortex --surfreg sphere.reg
                        mri_vol2surf --mov ${FC_dir}/${rest}_pp_sm0.nii.gz --reg ${func_reg_dir}/bbregister.dof6.dat --trgsubject ${fsaverage} --interp trilin --projfrac 0.5 --hemi ${hemi} --o ${FC_dir}/${rest}.pp.sm0.${fsaverage}.${hemi}.nii.gz --noreshape --cortex --surfreg sphere.reg
                        mris_fwhm --s ${fsaverage} --hemi ${hemi} --smooth-only --i ${gsFC_dir}/${rest}.pp.sm0.${fsaverage}.${hemi}.nii.gz --fwhm ${FWHM} --o ${gsFC_dir}/${rest}.pp.sm${FWHM}.${fsaverage}.${hemi}.nii.gz --mask ${func_dir}/mask/brain.${fsaverage}.${hemi}.nii.gz
                        mris_fwhm --s ${fsaverage} --hemi ${hemi} --smooth-only --i ${FC_dir}/${rest}.pp.sm0.${fsaverage}.${hemi}.nii.gz --fwhm ${FWHM} --o ${FC_dir}/${rest}.pp.sm${FWHM}.${fsaverage}.${hemi}.nii.gz --mask ${func_dir}/mask/brain.${fsaverage}.${hemi}.nii.gz
                done
        else
