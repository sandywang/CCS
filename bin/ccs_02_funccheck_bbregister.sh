...
check_funcbbregister_list=${dir}/scripts/check_funcbbregister.list
## Start scripts
...
do
	...
	convert -font helvetica -fill white -pointsize 36 -draw "text 20,20 '$title'" ${output_dir}/summary.png ${output_dir}/summary.png
 else
 	echo "BBR check has beeb run for ${subject}!"
 fi
	echo ${subject} >> ${check_funcbbregister_list}
done
