...
then
	3dcalc -a ${rest}.nii.gz[${TRstart}..${TRend}] -expr 'a' -prefix ${rest}_dr.nii.gz -datum float 2>&1
	3drefit -TR ${TR} ${rest}_dr.nii.gz 2>&1
fi
...
then    
	3dDespike -prefix ${rest}_dspk.nii.gz ${rest}_dr.nii.gz 2>&1
fi
...
then
	3dTshift -prefix ${rest}_ts.nii.gz -tpattern ${tpattern} -tzero 0 ${rest}_dspk.nii.gz 2>&1
	echo "Deobliquing ${subject}"
	3drefit -deoblique ${rest}_ts.nii.gz 2>&1
fi
...
then
	3dresample -orient RPI -inset ${rest}_ts.nii.gz -prefix ${rest}_ro.nii.gz 2>&1
fi
...
then
	3dTstat -mean -prefix ${rest}_ro_mean.nii.gz ${rest}_ro.nii.gz 2>&1 
fi
3dvolreg -Fourier -twopass -base ${rest}_ro_mean.nii.gz -zpad 4 -prefix ${rest}_mc.nii.gz -1Dfile ${rest}_mc.1D ${rest}_ro.nii.gz 2>&1

##5. Remove skull/edge detection
echo "Skull stripping ${subject}"
if [[ ! -f ${rest}_mask.nii.gz ]]
then
	3dAutomask -prefix ${rest}_mask.nii.gz -dilate 1 ${rest}_mc.nii.gz 2>&1
fi
...
then
	...
	3dcalc -a ${rest}_mc.nii.gz -b ${rest}_pp_mask.nii.gz -expr 'a*b' -prefix ${rest}_ss.nii.gz 2>&1
fi
