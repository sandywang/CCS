...
do
	...
	3dTsort -overwrite -prefix ${rest}_sm${fwhm}_rank.nii.gz -rank tmp_iso3mm.nii.gz 2>&1
	...
	then
		...
		applywarp --ref=${standard_template} --in=ReHo_sm${fwhm}.nii.gz --out=fnirt_ReHo_sm${fwhm}.nii.gz --warp=${anat_reg_dir}/highres2standard_ref_warp--premat=${func_reg_dir}/example_func2highres.mat
	else
		...
		applywarp --ref=${standard_template} --in=ReHo_sm${fwhm}.nii.gz --out=fnirt_ReHo_sm${fwhm}.nii.gz --warp=${anat_reg_dir}/highres2standard_warp --premat=${func_reg_dir}/example_func2highres.mat
	fi
	rm -rv tmp*.nii.gz
## END OF FWHM LOOP
done
