...
do
	...
	then
		...
		3dTcorrelate -pearson -polort -1 -prefix VMHC_sm${fwhm}.nii.gz ${vmhc_dir}/${rest}_sm${fwhm}_res2symmstandard.nii.gz tmp_LRflipped.nii.gz 2>&1
		## 3. Fisher Z transform map
		3dcalc -a VMHC_sm${fwhm}.nii.gz -expr 'log((1+a)/(1-a))/2' -prefix VMHC_sm${fwhm}_Zmap.nii.gz 2>&1
		if [ ${fwhm} -eq 0 ]
		then
		...
		echo ${nvols} volumes... 
		3dcalc -a VMHC_sm${fwhm}_Zmap.nii.gz -expr 'a*sqrt('${nvols}'-3)' -prefix VMHC_sm${fwhm}_Zstat.nii.gz 2>&1
