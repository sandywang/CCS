...
then 
	3dcopy ${func_dir}/${rest}_pp_mask.nii.gz ${func_seg_dir}/global_mask.nii.gz 2>&1
fi
...
then
	...
	slicer rendered_mask -S 3 6 csf_mask_fs.png
	title=${subject}.ccs.csf.fs.segment
	convert -font helvetica -fill white -pointsize 36 -draw "text 30,50 '$title'" csf_mask_fs.png csf_mask_fs.png
fi
...
then
	...
	slicer rendered_mask -S 3 6 wm_mask_fs.png
	title=${subject}.ccs.wm.fs.segment
	convert -font helvetica -fill white -pointsize 36 -draw "text 30,50 '$title'" wm_mask_fs.png wm_mask_fs.png
fi
rm -f rendered_mask.nii.gz
...
then
	...
	echo Please run lfcd_06_singlesubjectRFMRIparcels.m first!
fi
